...
| リベース済みトークン残高の表示         | 取引や流動性提供のためにUIでスケーリングされた残高を表示する。（バックエンドでは引き続き生の金額を使用可能）               | **P0** |
...
| 内部的に生の残高を追跡                 | オンチェーントランザクションではスケーリングされた残高ではなく生の残高を追跡する。これは長期的にはより正確で管理しやすい。 | **P1** |
| 価格フィードの適応                     | 現在の価格を表示するために価格フィードが使用される場所では、エンドユーザーにスケーリングされた価格を提供する。             | **P1** |
| 価格履歴グラフの表示                   | 価格グラフにスケーリングされた価格を反映させる。これには過去の価格を現在の乗数に再スケーリングすることも含まれる。         | **P1** |
| コストベースのスケーリング             | 1株あたりのコストは現在の乗数にスケーリングする必要がある。                                                                | **P1** |
