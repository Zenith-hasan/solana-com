...
| 요구사항                             | 설명                                                                                                                  | 우선순위 |
| :----------------------------------- | :-------------------------------------------------------------------------------------------------------------------- | :------- |
| 승수 업데이트 추적                   | 스케일드 UI 금액 확장을 사용하는 토큰의 승수 업데이트를 추적합니다.                                                   | **P0**   |
| 리베이스된 토큰 잔액 표시            | UI에서 거래 또는 유동성 제공을 위한 스케일된 잔액을 표시합니다. (백엔드는 여전히 원시 금액을 사용할 수 있음)          | **P0**   |
| 토큰 액션 지원                       | 최종 사용자는 UiAmount 잔액(승수 \* 원시 금액)으로 액션 금액을 입력해야 합니다.                                       | **P0**   |
| 과거 액션은 재스케일링되지 않아야 함 | 거래와 같은 과거 액션은 액션 시점의 정확한 스케일된 금액과 가격을 사용하여 표시해야 합니다.                           | **P1**   |
| 내부적으로 원시 잔액 추적            | 스케일된 잔액 대신 온체인 트랜잭션에 대한 원시 잔액을 추적합니다. 이는 장기적으로 더 정확하고 관리하기 쉬울 것입니다. | **P1**   |
| 가격 피드 적응                       | 현재 가격을 표시하는 데 가격 피드가 사용되는 모든 곳에서 최종 사용자에게 스케일된 가격을 제공합니다.                  | **P1**   |
| 가격 기록 그래프 표시                | 가격 그래프에 스케일된 가격을 반영합니다. 여기에는 과거 가격을 현재 승수로 재스케일링하는 것이 포함됩니다.            | **P1**   |
| 원가 기준 스케일링                   | 주당 비용은 현재 승수로 스케일링되어야 합니다.                                                                        | **P1**   |
