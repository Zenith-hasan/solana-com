...
- 当前乘数可以随时通过调用代币铸造合约读取。
  `getAccountInfo`。此外，如果设置了未来的乘数，这些信息也可以从代币铸造合约中获取。我们建议不要显示此乘数，因为这可能会让用户体验感到困惑。
...
| 要求                   | 描述                                                                       | 优先级 |
| :--------------------- | :------------------------------------------------------------------------- | :----- |
| 跟踪倍数更新           | 跟踪使用缩放 UI 数量扩展的代币的倍数更新。                                 | **P0** |
| 显示重新调整的代币余额 | 在 UI 上显示用于交易或流动性提供的缩放余额。（后端仍可使用原始金额）       | **P0** |
| 支持代币操作           | 最终用户应使用其 UiAmount 余额（倍数 \* 原始金额）输入操作金额。           | **P0** |
| 历史操作不应重新调整   | 历史操作（如交易）应使用操作时的准确缩放金额和价格显示。                   | **P1** |
| 内部跟踪原始余额       | 对于链上交易，应跟踪原始余额而非缩放余额。这将在长期内更加准确且易于管理。 | **P1** |
| 价格源适配             | 在任何使用价格源显示当前价格的地方，向最终用户提供缩放后的价格。           | **P1** |
| 显示价格历史图表       | 在价格图表中反映缩放后的价格。这包括将历史价格重新调整为当前倍数。         | **P1** |
| 缩放成本基础           | 每股成本应根据当前倍数进行缩放。                                           | **P1** |
